...
function DrawGrid(args) {
    ...
    //最外圈框
DrawGrid.prototype.drawRect = function(){
    var _this = this,
        grid    = _this.grid,
        ctx  = _this.ctx,
        x  = _this.option.x,
    ...
    //绘制线
DrawGrid.prototype.drawLine = function(fromX, fromY, toX, toY ,bold ,colorType){
    	var _this   = this, 
            ctx     = _this.ctx;
    	if(bold === 'diagonal'){
    		ctx.lineWidth = 1.5;
    ...
    //绘制头部格子
DrawGrid.prototype.drawHeader = function(){
    var _this   = this,
        ctx     = _this.ctx,
        grid    = _this.grid,
    ...
    //绘制头部文字
DrawGrid.prototype.drawHeaderText = function(i){
    var grid = 21;
    //期号
    if(i === 1){
    ...
    //绘制奖号
DrawGrid.prototype.drawTheNo = function(){
    var _this 	= this,
        i 		= 0,
...
DrawGrid.prototype.drawFooter = function(){
    var _this   = this,
        width   = _this.option.width;
    //绘制底部背景
    _this.ctx.beginPath();
...
DrawGrid.prototype.drawReady = function(){
     ...
     */
DrawGrid.prototype.drawArc = function(nums ,height){
    var _this 	= this,
        grid    = _this.grid,
        num1 = nums[0],
        num2 = nums[1],
...
DrawGrid.prototype.drawNums = function(data, height, nextItem){
    if(!data) return;
    var _this 	= this,
        grid    = _this.grid,
        num1 = data.nums[0],
 ...
 */
DrawGrid.prototype.drarDiagonal = function(fromX ,fromY, nextItem ,item ,type){
    if(nextItem){
        var _this 	= this,
            grid    = _this.grid,
    ...
    //绘制文字
DrawGrid.prototype.drawText = function(text ,x ,y ,type){
    ...
    switch(type){
        case 'head':
            ctx.font = "bold 20px AdobeHeitiStd";
            break;
        case 'issue':
            ctx.font = "17px AdobeHeitiStd";
            break;
        case 'normal':
            ctx.font = "bold 22px AdobeHeitiStd";
            break;
        case 'red':
            ctx.font = "bold 22px AdobeHeitiStd";
            break;
    ...
    //判断是否需要间隔线
DrawGrid.prototype.isBold = function(i){
    if(i == 1 || i == 4 || i == 10 || i == 26 || i == 28 || i == 30){
        return 'bold';
    }
    return null;
}
DrawGrid.prototype.refresh = function(){
